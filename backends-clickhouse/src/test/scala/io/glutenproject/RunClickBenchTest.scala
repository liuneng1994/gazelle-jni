...
// scalastyle:off
object RunClickBenchTest {

  def main(args: Array[String]): Unit = {
    // parquet or mergetree
    val fileFormat = "parquet"
    val libPath = "/home/admin1/github/ClickHouse/cmake-build-relwithdebinfo/utils/extern-local-engine/libch.so"
    if (!(new File(libPath)).exists()) System.exit(1)
    // TPCH data files path
    val dataFilesPath = "file:///home/admin1/data/hits"
//    if (!(new File(dataFilesPath)).exists()) System.exit(1)
    ...
    val queryPath = rootPath + "/clickbench/queries.sql"
    // which sql to execute
    val sqlNo = 29
    val queries = Source.fromFile(new File(queryPath), "UTF-8").getLines().toList
    val query = queries(sqlNo)
...
//      .config("spark.gluten.sql.substrait.plan.logLevel", "WARN")
    ...
    createHits(spark,  dataFilesPath)
    runSql(spark, query, executedCnt)
  }

  def createHits(spark: SparkSession, dataFilesPath: String): Unit = {
    val create_sql_file = this.getClass.getResource("/").getPath + "/clickbench/create.sql"
    val create_sql = Source.fromFile(new File(create_sql_file), "UTF-8")
    spark.sql(create_sql.mkString.replace("{}", s"$dataFilesPath"))
  }

  def runSql(spark: SparkSession, sqlStr: String, executedCnt: Int): Unit = {
    spark
      .sql(s"""
              |use default;
              |""".stripMargin)
