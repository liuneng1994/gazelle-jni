...
class GlutenExtensionExpressionSuite
  ...
  test("test ke bitmap index files") {
    ...
    for (i <- 0 until 2) {
      ...
      println(s"""
                 |----------------------------------------------------------------
                 |PreciseCountDistinct--read time: ${TimeStat.readTime.longValue()}
                 |PreciseCountDistinct--or time: ${TimeStat.orTime.longValue()}
                 |PreciseCountDistinct--or count: ${TimeStat.orCount.longValue()}
                 |PreciseCountDistinct--ser time: ${TimeStat.serTime.longValue()}
                 |PreciseCountDistinct--de time: ${TimeStat.deTime.longValue()}
                 |""".stripMargin)
  ...
  ignore("test bitmap_or") {
    ...
    /* val SQL1 = "select col1, bitmap_cardinality(bitmap_or(col2)) as col2 from t group by col1"
    spark.sql(SQL1).explain(false)
    compareResultsAgainstVanillaSpark(
      SQL1,
      true,
      df => {
    ...
    for (i <- 0 until 5) {
      val SQL2 = "select col1, ke_bitmap_or_cardinality(col2) as col2 from t group by col1"
  ...
  ignore("test bitmap_and_ids") {
    ...
    df0.createOrReplaceTempView("t")
    // val SQL1 =
    //   "select explode(ids) from (select col1, bitmap_and_ids(col2) as ids from t group by col1)"
  ...
  ignore("test explode and array") {
    ...
    withSQLConf(vanillaSparkConfs(): _*) {
      spark.sql(SQL1).show(1000, false)
    }

    withSQLConf(vanillaSparkConfs(): _*) {
      spark.sql(SQL2).show(1000, false)
    }
