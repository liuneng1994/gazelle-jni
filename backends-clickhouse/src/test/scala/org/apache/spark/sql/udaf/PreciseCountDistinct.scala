...
sealed abstract class BasicPreciseCountDistinct(
  ...
  override def merge(
    ...
    TimeStat.orTime.addAndGet(w.getNanoTime)
    TimeStat.orCount.addAndGet(1)
    w.stop()
    buffer
  }
  ...
  override def serialize(buffer: Roaring64NavigableMap): Array[Byte] = {
    val w = StopWatch.createStarted()
    val re = if (buffer.isInstanceOf[PlaceHolderBitmap]) {
      Array.empty[Byte]
    } else {
      buffer.repairAfterLazy()
      buffer.runOptimize()
      val size: Int = buffer.serializedSizeInBytes().asInstanceOf[Int]
      BitmapSerAndDeSer.serialize(buffer, size)
    }
    TimeStat.serTime.addAndGet(w.getNanoTime)
    w.stop()
    re
  }
  ...
  override def deserialize(bytes: Array[Byte]): Roaring64NavigableMap = {
    val w = StopWatch.createStarted()
    val re = if (bytes != null && bytes.nonEmpty) {
      BitmapSerAndDeSer.deserialize(bytes)
    } else {
      new PlaceHolderBitmap
    }
    TimeStat.deTime.addAndGet(w.getNanoTime)
    w.stop()
    re
  }
...
case class PreciseCountDistinct(
  ...
  override def update(buffer: Roaring64NavigableMap, input: InternalRow): Roaring64NavigableMap = {
    val w = StopWatch.createStarted()
    val colValue = child.eval(input)
    val rdata = deserialize(colValue.asInstanceOf[Array[Byte]])
    val t = w.getNanoTime
    TimeStat.readTime.addAndGet(t)
    buffer.naivelazyor(rdata)
    TimeStat.orTime.addAndGet((w.getNanoTime - t))
    TimeStat.orCount.addAndGet(1)
    w.stop()
    buffer
  }
  ...
  override def update(buffer: Roaring64NavigableMap, input: InternalRow): Roaring64NavigableMap = {
    ...
    if (buffer.isInstanceOf[PlaceHolderBitmap]) {
      ...
      if (!buffer.isEmpty) {
        ...
        buffer.and(new PlaceHolderBitmap)
      }
      buffer
    }
  }
...
case class PreciseCardinality(override val child: Expression)
  ...
  override def nullSafeEval(input: Any): Long = {
    ...
    BitmapSerAndDeSer.deserialize(data).getLongCardinality
  }
...
@SerialVersionUID(1)
case class PreciseBitmapBuildPushDown(
    child: Expression,
    mutableAggBufferOffset: Int = 0,
    inputAggBufferOffset: Int = 0)
  extends BasicPreciseCountDistinct(child, mutableAggBufferOffset, inputAggBufferOffset) {
...
object TimeStat {
  ...
  val orTime = new AtomicLong(0)
  val deTime = new AtomicLong(0)
  val serTime = new AtomicLong(0)
  val orCount = new AtomicLong(0)
}
