...
class HashJoinMetricsUpdater(val metrics: Map[String, SQLMetric])
  extends MetricsUpdater with Logging {
  override def updateNativeMetrics(opMetrics: IOperatorMetrics): Unit = {
    try {
      ...
      case e: Throwable =>
        logError(
          s"HashJoinMetricsUpdater Updating native metrics failed due to ${e.getCause}.")
    }
  }
}
